# MBTI Glass Bowls Sheet v1

## File

- `assets/bowls/mbti-glass-bowls-sheet-v1.png`

## Source design

- Handmade transparent glass matcha bowl.
- Rounded bowl silhouette with a subtle pouring lip.
- Hammered / rippled clear-glass texture.
- Blue speckles through the bowl body.
- Pale blue dots and blue lower fragments for the source-inspired ISFJ / ISFP
  bowls.
- No hand, fingers, background, or external prop should be included.

## MBTI grid order

The sheet is a 4x4 grid in row-major order:

| Row | Col 1 | Col 2 | Col 3 | Col 4 |
| --- | --- | --- | --- | --- |
| 1 | ISTJ | ISFJ | INFJ | INTJ |
| 2 | ISTP | ISFP | INFP | INTP |
| 3 | ESTP | ESFP | ENFP | ENTP |
| 4 | ESTJ | ESFJ | ENFJ | ENTJ |

## Fixed design constraints

- ISFJ and ISFP must keep the source design colors and pattern: clear glass,
  blue speckles, pale blue dots, and blue lower fragments only. They must not
  include pink, copper, orange, green, purple, or any non-blue accent color.
- Other MBTI bowls may vary color palettes while keeping the same glass bowl
  silhouette and handmade speckled-glass style.

## Next asset tasks

- Slice the sheet into 16 consistent frames.
- Create a Phaser atlas JSON using the MBTI type as each frame name.
- Optionally export individual transparent PNGs for UI and ending screens.
