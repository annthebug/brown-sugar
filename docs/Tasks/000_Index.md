...
- Task 005（黑糖移動與能力）已實作完成，並修復了 `GameScene` 中殘留的 Task 004 靜態黑糖 placeholder 與 `Player` 重疊造成的「畫面出現兩隻黑糖」視覺問題。
- Memory Shard 目前仍為點擊拾取（Task 006 遺留待辦：改為與 `Player` 碰撞自動拾取），章節關卡、Howler 音訊服務、Firebase Sync、PWA、Vercel 部署與成就系統尚未進入核心實作。
